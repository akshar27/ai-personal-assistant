...
_RULES: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"(?i)\b(reveal|print|repeat|show)\s+(me\s+)?(your|the)\s+(system\s+)?(prompt|instructions|rules)"), "prompt-leak attempt"),
    (re.compile(r"(?i)\bsystem\s+prompt\b"), "system-prompt probe"),
    (re.compile(r"(?im)^\s*(assistant|system|developer)\s*:"), "forged role turn"),
    (re.compile(r"(?i)forward\s+(this|the|all)\s+(email|thread|message|conversation|mail)s?\s+to\b"), "exfiltration instruction"),
    (re.compile(r"(?i)\bsend\s+(an?\s+)?(email|message|reply)\b[^.\n]{0,60}\bto\b[^.\n]{0,40}@"), "embedded send instruction"),
    (re.compile(r"(?i)\bdelete\s+(all|every|the)\b[^.\n]{0,40}\b(event|calendar|email|message|file)s?\b"), "destructive instruction"),
